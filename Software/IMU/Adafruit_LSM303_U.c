...
void _accel_read(struct accel_t *accel) {
    ...
    i2c_read(LSM303_ADDRESS_ACCEL,LSM303_REGISTER_ACCEL_OUT_X_L_A,1,buff);
    uint8_t xlo = buff[0];

    i2c_read(LSM303_ADDRESS_ACCEL,LSM303_REGISTER_ACCEL_OUT_X_H_A,1,buff);
    uint8_t xhi = buff[0];

    i2c_read(LSM303_ADDRESS_ACCEL,LSM303_REGISTER_ACCEL_OUT_Y_L_A,1,buff);
    uint8_t ylo = buff[0];

    i2c_read(LSM303_ADDRESS_ACCEL,LSM303_REGISTER_ACCEL_OUT_Y_H_A,1,buff);
    uint8_t yhi = buff[0];

    i2c_read(LSM303_ADDRESS_ACCEL,LSM303_REGISTER_ACCEL_OUT_Z_L_A,1,buff);
    uint8_t zlo = buff[0];

    i2c_read(LSM303_ADDRESS_ACCEL,LSM303_REGISTER_ACCEL_OUT_Z_H_A,1,buff);
    uint8_t zhi = buff[0];
...
bool accel_create(struct accel_t **ret_accel, int32_t sensorID)
{
  ...
  // Enable the accelerometer (100Hz)
    i2c_write(LSM303_ADDRESS_ACCEL, LSM303_REGISTER_ACCEL_CTRL_REG1_A, 0x57);
  
  // LSM303DLHC has no WHOAMI register so read CTRL_REG1_A back to check
  // if we are connected or not
    uint8_t reg1_a[1];
    i2c_read(LSM303_ADDRESS_ACCEL, LSM303_REGISTER_ACCEL_CTRL_REG1_A,1,reg1_a);
  if (reg1_a[0] != 0x57)
  {
    return false;
  }  
...
void _mag_read(struct mag_t *mag)
{
    ...
    i2c_read(LSM303_ADDRESS_MAG,LSM303_REGISTER_MAG_OUT_X_L_M,1,buff);
    uint8_t xlo = buff[0];

    i2c_read(LSM303_ADDRESS_MAG,LSM303_REGISTER_MAG_OUT_X_H_M,1,buff);
    uint8_t xhi = buff[0];

    i2c_read(LSM303_ADDRESS_MAG,LSM303_REGISTER_MAG_OUT_Y_L_M,1,buff);
    uint8_t ylo = buff[0];

    i2c_read(LSM303_ADDRESS_MAG,LSM303_REGISTER_MAG_OUT_Y_H_M,1,buff);
    uint8_t yhi = buff[0];

    i2c_read(LSM303_ADDRESS_MAG,LSM303_REGISTER_MAG_OUT_Z_L_M,1,buff);
    uint8_t zlo = buff[0];

    i2c_read(LSM303_ADDRESS_MAG,LSM303_REGISTER_MAG_OUT_Z_H_M,1,buff);
    uint8_t zhi = buff[0];
...
bool mag_create( struct mag_t **ret_mag, int32_t sensorID)
{
  ...
  // Enable I2C
  //Wire.begin();
  int fd = i2c_open(LSM303_ADDRESS_MAG);
  if (fd == -1) {
	  return false;
  }
  ...
  i2c_write(LSM303_ADDRESS_MAG, LSM303_REGISTER_MAG_MR_REG_M, 0x00);

  // LSM303DLHC has no WHOAMI register so read CRA_REG_M to check
  // the default value (0b00010000/0x10)
  uint8_t reg1_a[1];
  i2c_read(LSM303_ADDRESS_MAG, LSM303_REGISTER_MAG_CRA_REG_M,1,reg1_a);
  if (reg1_a[0] != 0x10)
  {
    return false;
  }
...
{
  i2c_write(LSM303_ADDRESS_MAG, LSM303_REGISTER_MAG_CRB_REG_M, (byte)gain);
...
void mag_setRate(struct mag_t *mag, lsm303MagRate rate)
{
    ...
    i2c_write(LSM303_ADDRESS_MAG, LSM303_REGISTER_MAG_CRA_REG_M, reg_m);
}
...
bool mag_getEvent(struct mag_t *mag, sensors_event_t* event) {
  ...
  while(!readingValid)
  {
    ...
    uint8_t reg_mg[1];
    i2c_read(LSM303_ADDRESS_MAG, LSM303_REGISTER_MAG_SR_REG_Mg,1,reg_mg);
