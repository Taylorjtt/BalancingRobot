...
bool gyro_create( struct gyro_t **ret_gyro, int32_t sensorID, gyroRange_t rng) {
  ...
  uint8_t buff[1];
  i2c_read(L3GD20_ADDRESS,GYRO_REGISTER_WHO_AM_I,1,buff);
  uint8_t id = buff[0];
  ...
  i2c_write(L3GD20_ADDRESS, GYRO_REGISTER_CTRL_REG1, 0x00);
  i2c_write(L3GD20_ADDRESS, GYRO_REGISTER_CTRL_REG1, 0x0F);
  /* ------------------------------------------------------------------ */
  ...
  switch(rng)
  {
    case GYRO_RANGE_250DPS:
      i2c_write(L3GD20_ADDRESS, GYRO_REGISTER_CTRL_REG4, 0x00);
      break;
    case GYRO_RANGE_500DPS:
        i2c_write(L3GD20_ADDRESS, GYRO_REGISTER_CTRL_REG4, 0x10);
      break;
    case GYRO_RANGE_2000DPS:
        i2c_write(L3GD20_ADDRESS, GYRO_REGISTER_CTRL_REG4, 0x20);
      break;
  }
  /* ------------------------------------------------------------------ */
...
bool gyro_getEvent(struct gyro_t *gyro, sensors_event_t* event)
{
  ...
  {
      uint8_t buff[1];
      i2c_read(L3GD20_ADDRESS,GYRO_REGISTER_OUT_X_L,1,buff);
      uint8_t xlo = buff[0];

      i2c_read(L3GD20_ADDRESS,GYRO_REGISTER_OUT_X_H,1,buff);
      uint8_t xhi = buff[0];

      i2c_read(L3GD20_ADDRESS,GYRO_REGISTER_OUT_Y_L,1,buff);
      uint8_t ylo = buff[0];

      i2c_read(L3GD20_ADDRESS,GYRO_REGISTER_OUT_Y_H,1,buff);
      uint8_t yhi = buff[0];

      i2c_read(L3GD20_ADDRESS,GYRO_REGISTER_OUT_Z_L,1,buff);
      uint8_t zlo = buff[0];

      i2c_read(L3GD20_ADDRESS,GYRO_REGISTER_OUT_Z_H,1,buff);
      uint8_t zhi = buff[0];
           ...
           (event->gyro.z >= 32760) | (event->gyro.z <= -32760) )
      {
        ...
        switch(gyro->range)
        {
          case GYRO_RANGE_500DPS:
            ...
            i2c_write(L3GD20_ADDRESS, GYRO_REGISTER_CTRL_REG1, 0x0F);
            i2c_write(L3GD20_ADDRESS, GYRO_REGISTER_CTRL_REG4, 0x20);
            i2c_write(L3GD20_ADDRESS, GYRO_REGISTER_CTRL_REG5, 0x80);
            readingValid = false;
