 ...
 * @brief Converts a quaternion to Euler angles in degrees.
 * @param quaternion Quaternion.
 * @return Euler angles in rads.
 */
static inline FusionEuler FusionQuaternionToEulerRadians(const FusionQuaternion quaternion) {
#define Q quaternion.element
    const float halfMinusQySquared = 0.5f - Q.y * Q.y; // calculate common terms to avoid repeated operations
    FusionEuler euler;
    euler.angle.roll = atan2f(Q.w * Q.x + Q.y * Q.z, halfMinusQySquared - Q.x * Q.x);
    euler.angle.pitch = FusionAsin(2.0f * (Q.w * Q.y - Q.z * Q.x));
    euler.angle.yaw = atan2f(Q.w * Q.z + Q.x * Q.y, halfMinusQySquared - Q.z * Q.z);
    return euler;
#undef Q
}
